import.meta.url = "file:///Users/kentaylor/.pi/agent/extensions/loop.ts";
...
const LOOP_PRESETS = [
    ...
    {
        value: "self",
        label: "Self driven (agent decides)",
        description: ""
    }
];
...
`;
function buildPrompt(mode, condition) {
    switch(mode){
        case "tests":
            return ("Run all tests. If they are passing, call the signal_loop_success tool. " + "Otherwise continue until the tests pass.");
        case "custom":
            {
                const customCondition = condition?.trim() || "the custom condition is satisfied";
                return (`Continue until the following condition is satisfied: ${customCondition}. ` + "When it is satisfied, call the signal_loop_success tool.");
            }
        case "self":
            return "Continue until you are done. When finished, call the signal_loop_success tool.";
    }
}
function summarizeCondition(mode, condition) {
    switch(mode){
        case "tests":
            return "tests pass";
        case "custom":
...
function getConditionText(mode, condition) {
    switch(mode){
        case "tests":
            return "tests pass";
        case "custom":
...
async function selectSummaryModel(ctx) {
...
async function summarizeBreakoutCondition(ctx, mode, condition) {
    ...
    const response = await complete(selection.model, {
    ...
    const summary = response.content.filter((c)=>c.type === "text").map((c)=>c.text).join(" ").replace(/\s+/g, " ").trim();
    if (!summary) return fallback;
    return summary.length > 60 ? `${summary.slice(0, 57)}...` : summary;
}
function getCompactionInstructions(mode, condition) {
    const conditionText = getConditionText(mode, condition);
...
export default function loopExtension(pi) {
    ...
    function setLoopState(state, ctx) {
        loopState = state;
        persistState(state);
        updateStatus(ctx, state);
    }
    function clearLoopState(ctx) {
        const cleared = {
            active: false
        };
        loopState = cleared;
    ...
    function breakLoop(ctx) {
        clearLoopState(ctx);
        ctx.ui.notify("Loop ended", "info");
    }
    function wasLastAssistantAborted(messages) {
    ...
    function triggerLoopPrompt(ctx) {
    ...
    async function showLoopSelector(ctx) {
        const items = LOOP_PRESETS.map((preset)=>({
                value: preset.value,
                label: preset.label,
                description: preset.description
            }));
        const selection = await ctx.ui.custom((tui, theme, _kb, done)=>{
            ...
            if (!loopState.active) {
                return {
                    content: [
                        {
                            type: "text",
                            text: "No active loop is running."
                        }
                    ],
                    details: {
                        active: false
    ...
    pi.registerCommand("loop", {
        ...
        handler: async (args, ctx)=>{
            ...
            if (loopState.active) {
                const confirm = ctx.hasUI ? await ctx.ui.confirm("Replace active loop?", "A loop is already active. Replace it?") : true;
                if (!confirm) {
    ...
    async function restoreLoopState(ctx) {
