import.meta.url = "file:///Users/kentaylor/.pi/agent/extensions/init.ts";
...
function buildPrompt(cwd, agentsExists, includeWorkspaces) {
    const filePath = path.join(cwd, AGENTS_FILE);
    const workspaceNote = includeWorkspaces ? "- If this is a monorepo root, also consider whether missing workspace-level AGENTS.md files should be created where helpful.\n" : "";
    const existingMode = agentsExists ? `There is already an ${AGENTS_FILE} at \`${filePath}\`. Improve it sparingly instead of replacing user-authored guidance.` : `Create a new ${AGENTS_FILE} at \`${filePath}\`.`;
    return [
        `${existingMode}`,
        "",
            ...
            const items = [
                {
                    value: "--w",
                    label: "--w",
                    description: "Mention workspace-aware monorepo initialization"
                },
                {
                    value: "--help",
                    label: "--help",
