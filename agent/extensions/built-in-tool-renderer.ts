...
export default function (pi: ExtensionAPI) {
	...
	// ── Expand mode toggle ──
	// true  = always show content preview (default)
	// false = only show preview when user presses ctrl+o
	let expandByDefault = true;

	// Signal that renderer loaded
	console.log(`[built-in-tool-renderer] LOADED — expandByDefault=${expandByDefault}`);

	pi.registerCommand("tool-expand", {
		description: "Toggle tool output: always show preview vs. ctrl+o only",
		handler: async (_args, ctx) => {
			expandByDefault = !expandByDefault;
			const mode = expandByDefault ? "ALWAYS SHOW preview" : "ctrl+o TO EXPAND";
			console.log(`[built-in-tool-renderer] ${mode}`);
			ctx.ui.notify(`Tool output: ${mode}`, expandByDefault ? "success" : "warning");
		},
	});
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme, _context) {
			...
			// Show preview: always if expandByDefault, or only if user expanded via ctrl+o
			const showPreview = expandByDefault || expanded;
			const previewLines = expanded ? 30 : 15; // more lines when explicitly expanded
			if (showPreview) {
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme, _context) {
			...
			const showPreview = expandByDefault || expanded;
			const previewLines = expanded ? 40 : 20;
			if (showPreview) {
	...
	pi.registerTool({
		...
		renderCall(args, theme, _context) {
			let text = theme.fg("toolTitle", theme.bold("✏️ EDIT "));
			text += theme.fg("accent", args.path);
			return new Text(text, 0, 0);
		},
		...
		renderResult(result, { expanded, isPartial }, theme, _context) {
			...
			const showPreview = expandByDefault || expanded;
			const previewLines = expanded ? 60 : 30;
			if (showPreview) {
				...
				if (diffLines.length > previewLines) {
					text += `\n${theme.fg("muted", `... ${diffLines.length - previewLines} more diff lines`)}`;
				}
			}
