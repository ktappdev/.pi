...
4. `Action Steps:` numbered list of what the specialist must do. For complex tasks:
   ...
   - Describe changes in plain language using line number ranges (e.g., "lines 30-45: change the validation to accept null"). The agent will read the file. Only use `Find:` / `Replace with:` code blocks for one-liners where an exact string match is critical (e.g., importing a symbol, toggling a flag). Never paste entire files or large code blocks.
